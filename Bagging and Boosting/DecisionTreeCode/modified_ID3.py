...
class ID3:
	...
	def buildTree(self, examples, attributes, used_attributes, depth):
		weight_index =  attributes['WEIGHTS'].index
		if depth > self.depth_limit:
			return createLeaf(majority(examples, weight_index), depth + 1)
	...
	def testAttributes(self, examples, attributes, used_attributes):
		...
		best_attribute = -1
		weight_index =  attributes['WEIGHTS'].index
		ex_entropy = discreteEntropy(examples, attributes['CLASS'],weight_index)['hY_X']
		...
		for attribute in attributes.values():
			if attribute.type == Feature.Type.BINARY or attribute.type == Feature.Type.NOMINAL:
				...
				if attribute.name not in used_attributes.keys():
					this_entropy = discreteEntropy(examples, attribute, weight_index)
				else:
					continue
			elif attribute.type == Feature.Type.CONTINUOUS:
				this_entropy = continuousEntropy(examples,attribute, used_attributes, weight_index)
			else:
				continue
	...
	global majority
	def majority(examples, weight_index):
		pos_count = 0
		neg_count = 0
		for ex in examples:
			if ex[-1] == 1:
				pos_count += ex[weight_index]
			else:
				neg_count += ex[weight_index]
	...
	# This is modified from the original code, so that instead of counting the
	# frequencies, we are adding up the weights of each example
	global discreteEntropy
	def discreteEntropy(examples,attribute, weight_index):
		...
		for ex in examples:
			...
			if ex[-1] == 0:
				frequencies[ex[index]][0] += ex[weight_index]
			else:
				frequencies[ex[index]][1] += ex[weight_index]
	...
	def continuousEntropy(examples, attribute, used_attributes, weight_index):
		...
		for ex in examples:
			if ex[-1] == 1:
				tot_pos += ex[weight_index]
			else:
				tot_neg += ex[weight_index]
		...
		while curr_start < ex_size and prev_end < ex_size:
			...
			leq_buckets[prev_start_Class] += examples[prev_start][weight_index]
			greater_buckets[prev_start_Class] -= examples[prev_start][weight_index]
			...
			while prev_start_Val == prev_end_Val:
				leq_buckets[prev_end_Class] += examples[prev_end][weight_index]
				greater_buckets[prev_end_Class] -= examples[prev_end][weight_index]
				prev_end += 1
				if prev_end >= ex_size:
					break
				prev_end_Class = int(examples[prev_end][-1])
