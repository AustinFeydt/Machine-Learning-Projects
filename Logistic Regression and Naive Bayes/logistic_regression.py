import numpy as np
def logistic_regression(examples, alt_examples, lda):
	w = np.zeros(np.size(examples,1))
	return weight_gradient(examples, alt_examples, w, lda)

# stochastic gradient descent to find optimal weights
def weight_gradient(examples, alt_examples, w, lda):
	eta = 0.00001
	ll = 0
	for i in xrange(5000):
		for j in xrange(np.size(examples,0)):
			...
			gradient = alt_examples[j]*out_err + weight_contr
